...
$(document).ready(() => {
	...
	$('#competencias .lista-habilidades:first li:not(:first) .corpo').slideUp();
	$('#competencias .lista-habilidades li .cabecalho').click( e => {
		let corpo = $(e.target).closest('.habilidade').find('.corpo');
		let sinal = $(e.target).closest('.habilidade').find('.cabecalho .sinal-expandir i');
		corpo.slideToggle();
